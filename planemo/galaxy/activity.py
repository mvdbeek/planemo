...
def _execute(ctx, config, runnable, job_path, **kwds):
    ...
    ctx.vlog("collecting invocation details complete")
    run_response.collect_invocation_details()
...
class GalaxyWorkflowRunResponse(GalaxyBaseRunResponse):
    ...
    def collect_invocation_details(self):
        ...
        for step in self._invocation['steps']:
            ...
            workflow_step = gi.invocations.show_invocation_step(self._invocation_id, step['workflow_step_id'])
